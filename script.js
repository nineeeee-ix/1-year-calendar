const calendar = document.getElementById("calendar");
    ...
    function openModal(dateStr) {
      ...
      eventDate.textContent = formatDate(dateStr);
    ...
    cancelEvent.onclick = closeModalFunc;
    ...
    saveEvent.onclick = () => {
      const dateStr = eventDate.textContent.split(', ')[1]; // Get the date from formatted string
      const isoDateStr = formatDateToISO(dateStr);
      const newEvent = newEventInput.value.trim();
      
      if (newEvent) {
        events[isoDateStr] = newEvent;
    ...
    document.addEventListener('keydown', function(e) {
      if (eventModal.style.display === 'block') {
        if (e.key === 'Escape') {
          closeModalFunc();
        } else if (e.key === 'Enter' && document.activeElement === newEventInput) {
          saveEvent.click();
        }
      }
